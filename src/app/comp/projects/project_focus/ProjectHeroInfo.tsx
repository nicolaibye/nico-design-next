      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-center w-full h-full px-10 md:px-20">
        {info.cards.map((card, i) => (
          <button
            ...
            className={`${CARD_OUTLINE_COLORS[i]} h-full p-8 xl:p-12 rounded-xl w-full backdrop-blur-[9px] font-lexend ${activeCard === i ? `${CARD_TEXT_COLORS_ACTIVE[i]}` : `${CARD_TEXT_COLORS[i]}`} text-left w-full hover:scale-[1.02] cursor-pointer ${activeCard === i ? `${CARD_COLORS[i]}` : `${CARD_COLORS[i]}/10`}`}
          >
