...
const ProjectHeroInfo = ({ info }: { info: ProjectInfo }) => {
  ...
  return (
    <div className="project-opening">
      ...
      {/* Mobile stats */}
      <ul className="md:hidden flex flex-col gap-3 w-full text-sm font-lexend font-light text-black-Mirage my-10 dark:text-white-LinkWater px-20">
        {info.stats.map((stat) => (
      ...
      {/* Cards */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-center w-full h-full px-20">
        {info.cards.map((card, i) => (
          <button
            key={card.title}
            onClick={() => setActiveCard(activeCard === i ? null : i)}
            className={`${CARD_OUTLINE_COLORS[i]} h-full p-8 xl:p-12 rounded-xl backdrop-blur-[9px] font-lexend ${activeCard === i ? `${CARD_TEXT_COLORS_ACTIVE[i]}` : `${CARD_TEXT_COLORS[i]}`} text-left w-full hover:scale-[1.02] cursor-pointer ${activeCard === i ? `${CARD_COLORS[i]}` : `${CARD_COLORS[i]}/10`}`}
          >
